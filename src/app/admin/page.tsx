...
export default async function AdminPage({ searchParams }: { searchParams: Search }) {
  ...
  return <><div className="title-row"><div><p className="eyebrow">Отчеты</p><h1>Статистика сборки</h1></div><SubmitForm action={updatePrice} className="price-form"><label>Ставка, ₽ <input name="price" type="number" min="0" step="0.01" defaultValue={price} required /></label></SubmitForm></div><PeriodFilter preset={preset} from={from} to={safeTo} />{error ? <p className="error">Не удалось загрузить статистику</p> : <div className="table-wrap"><table><thead><tr><th>Сотрудник</th><th>Заказов</th><th>Сумма</th><th>Дублей</th><th>Too fast</th></tr></thead><tbody>{(rows ?? []).map((row: { id: string; name: string; successful: number | string; duplicates: number | string; too_fast: number | string }) => { const success = Number(row.successful); return <tr key={row.id}><td><Link href={`/admin/employees/${row.id}?from=${from}&to=${safeTo}`}>{row.name}</Link></td><td>{success}</td><td>{(success * price).toLocaleString("ru-RU")} ₽</td><td>{Number(row.duplicates)}</td><td>{Number(row.too_fast)}</td></tr>; })}</tbody></table></div>}</>;
}
