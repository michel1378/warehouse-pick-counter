...
async function view(employeeId: string, name: string, shift?: Record<string, unknown> | null) {
  ...
  return { id: current.id, employeeName: name, status: current.status, startedAt: current.started_at, endedAt: current.ended_at, pauseStartedAt, activeSeconds, totalSeconds, pauseSeconds, pauseCount: Number(current.pause_count), orders: current.status === "finished" ? Number(current.orders_count) : count ?? 0, earnings: current.status === "finished" ? Number(current.earnings) : (count ?? 0) * price, medianIntervalSeconds: current.median_interval_seconds == null ? null : Number(current.median_interval_seconds) };
}
...
export async function POST(request: NextRequest) {
  ...
  if (parsed.data.action === "start") {
    ...
    const [{ data: scans }, { data: pauses }, { data: setting }] = await Promise.all([db.from("scans").select("scanned_at").eq("shift_id", shift.id).order("scanned_at"), db.from("work_shift_pauses").select("started_at,ended_at").eq("shift_id", shift.id), db.from("settings").select("price_per_order").eq("id", 1).single()]);
    const intervals: number[] = []; for (let i = 1; i < (scans?.length ?? 0); i++) { const a = new Date(scans![i - 1].scanned_at).getTime(), b = new Date(scans![i].scanned_at).getTime(); if (!(pauses ?? []).some(p => new Date(p.started_at).getTime() < b && new Date(p.ended_at ?? now).getTime() > a)) intervals.push((b - a) / 1000); } intervals.sort((a,b)=>a-b); const median = intervals.length ? intervals.length % 2 ? intervals[(intervals.length-1)/2] : (intervals[intervals.length/2-1]+intervals[intervals.length/2])/2 : null;
    const total = Math.floor((now.getTime() - new Date(shift.started_at).getTime()) / 1000), orders = scans?.length ?? 0; shift = (await db.from("work_shifts").update({ status: "finished", ended_at: now.toISOString(), active_seconds: Math.max(0, total - Number(shift.pause_seconds)), orders_count: orders, earnings: orders * Number(setting?.price_per_order ?? 0), median_interval_seconds: median }).eq("id", shift.id).select("*").single()).data;
  }
