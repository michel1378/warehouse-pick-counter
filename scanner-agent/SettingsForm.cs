...
internal sealed class SettingsForm : Form
{
    ...
    private readonly TextBox _employee = new() { Dock = DockStyle.Fill, PlaceholderText = "UUID сотрудника или PIN" };
    ...
    private readonly Label _identify = new() { AutoSize = true, Text = "" };
    private bool _waiting;
    public AgentConfig? Result { get; private set; }
    public string Token => _token.Text;

    public SettingsForm(AgentConfig? current, RawInput raw)
    {
        Text = "Настройка складского сканера"; Width = 700; Height = 390; StartPosition = FormStartPosition.CenterScreen; MaximizeBox = false;
        var grid = new TableLayoutPanel { Dock = DockStyle.Fill, Padding = new Padding(18), ColumnCount = 2, RowCount = 7 };
        grid.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 190)); grid.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100));
        AddRow(grid, 0, "URL сайта / API", _url); AddRow(grid, 1, "Сотрудник (UUID или PIN)", _employee); AddRow(grid, 2, "API token", _token); AddRow(grid, 3, "HID-сканер", _device);
        ...
        var identify = new Button { Text = "Определить сканер", AutoSize = true }; identify.Click += (_, _) => { _waiting = true; _identify.Text = "Отсканируйте любой штрихкод…"; }; var panel = new FlowLayoutPanel { Dock = DockStyle.Fill, AutoSize = true }; panel.Controls.Add(identify); panel.Controls.Add(_identify); grid.Controls.Add(panel, 1, 5);
        ...
        raw.KeyReceived += OnRawKey;
        FormClosed += (_, _) => raw.KeyReceived -= OnRawKey;
    }
    private static void AddRow(TableLayoutPanel grid, int row, string label, Control control) { grid.Controls.Add(new Label { Text = label, AutoSize = true, Anchor = AnchorStyles.Left }, 0, row); grid.Controls.Add(control, 1, row); }
    ...
    private void OnRawKey(string device, Keys key) { if (!_waiting || key is Keys.ShiftKey or Keys.LShiftKey or Keys.RShiftKey) return; BeginInvoke(() => { _waiting = false; if (!_device.Items.Contains(device)) _device.Items.Add(device); _device.SelectedItem = device; _identify.Text = "Сканер определён"; }); }
