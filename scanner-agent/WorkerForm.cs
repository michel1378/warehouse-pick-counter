...
internal sealed class WorkerForm : Form
{
    ...
    private readonly Button _start = new() { Text = "Начать смену", AutoSize = true }, _pause = new() { Text = "Пауза", AutoSize = true }, _resume = new() { Text = "Продолжить", AutoSize = true }, _finish = new() { Text = "Завершить смену", AutoSize = true };
    ...
    public WorkerForm()
    {
        ...
        var buttons = new FlowLayoutPanel { Dock = DockStyle.Fill, AutoSize = true }; buttons.Controls.AddRange([_start, _pause, _resume, _finish]); grid.Controls.Add(buttons, 0, 7); grid.SetColumnSpan(buttons, 2);
        ...
        _start.Click += (_, _) => ShiftActionRequested?.Invoke("start"); _pause.Click += (_, _) => ShiftActionRequested?.Invoke("pause"); _resume.Click += (_, _) => ShiftActionRequested?.Invoke("resume"); _finish.Click += (_, _) => ShiftActionRequested?.Invoke("finish");
