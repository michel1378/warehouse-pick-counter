...
{
    private readonly Label _employee = V("—"), _orders = V("0"), _earnings = V("0 ₽"), _scanner = V("Готов"), _server = V("Проверка…"), _timer = V("Смена не начата"), _median = V("—"), _notice = V("");
    ...
    public WorkerForm()
    {
        ...
        var grid = new TableLayoutPanel { Dock = DockStyle.Fill, Padding = new Padding(16), ColumnCount = 2, RowCount = 10 };
        ...
        _notice.Font = new Font(Font, FontStyle.Bold); grid.Controls.Add(_notice, 0, 7); grid.SetColumnSpan(_notice, 2);
        var buttons = new FlowLayoutPanel { Dock = DockStyle.Fill, AutoSize = true }; buttons.Controls.AddRange([_start, _pause, _resume, _finish, _settings]); grid.Controls.Add(buttons, 0, 8); grid.SetColumnSpan(buttons, 2);
        var top = new CheckBox { Text = "Поверх остальных окон", AutoSize = true }; top.CheckedChanged += (_, _) => TopMost = top.Checked; grid.Controls.Add(top, 0, 9); grid.SetColumnSpan(top, 2); Controls.Add(grid);
