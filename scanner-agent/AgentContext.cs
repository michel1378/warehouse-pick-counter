...
internal sealed class AgentContext : ApplicationContext
{
    ...
    private void ShowWorker() { if (_worker is null || _worker.IsDisposed) { _worker = new WorkerForm(); _worker.ShiftActionRequested += async action => await ChangeShift(action); } _worker.ShowFront(); UpdateTooltip("Работает"); }
    ...
    private async Task RestoreShift() { try { _shift = await _api.GetShiftAsync(_config); _worker?.SetConnection(true); _worker?.SetShift(_shift); } catch { _worker?.SetConnection(false); } }
    private async Task ChangeShift(string action)
    {
        ...
        catch (Exception ex) { _worker?.SetConnection(false); MessageBox.Show($"Не удалось изменить смену: {ex.Message}"); }
    }
    ...
    private async Task FlushQueue(bool announce = false)
    {
        ...
        try
        {
            ...
            {
                ScanResponse response; try { response = await _api.SendAsync(_config, item); } catch { SetOffline(); return; }
